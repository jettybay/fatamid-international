    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-sm shadow-sm border-b border-gray-100">
      <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">
        ...
        <button
          ...
          className={`md:hidden p-3 rounded-xl transition-all ${
            isSoftwarePath
              ? 'text-blue-600 hover:bg-blue-50 hover:shadow-md border border-blue-100 hover:border-blue-200'
              : 'text-green-700 hover:bg-green-100 hover:shadow-md border border-green-100 hover:border-green-200'
          }`}
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className={`flex flex-col p-6 space-y-2 backdrop-blur-sm ${
              isSoftwarePath ? 'bg-blue-50/80 border-t-2 border-blue-200' : 'bg-green-50/80 border-t-2 border-green-200'
            }`}>
              {navItems.map((item) => {
                const isActive = pathname === item.href
                return (
                  <Link
                    ...
                    className={`block py-4 px-6 rounded-2xl text-lg font-semibold transition-all shadow-sm ${
                      isActive
                        ...
                        : `${
                            isSoftwarePath
                              ? 'text-blue-700 hover:bg-blue-100 hover:text-blue-800 hover:shadow-md'
